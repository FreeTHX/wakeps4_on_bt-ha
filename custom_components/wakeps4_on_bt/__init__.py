...
def setup(hass, config):
    ...
    def send_magic_packet(call):
        ...
        if wakeps4onbt.send_magic_packet(adapter,
                                         ps4_address_toconnectto,
                                         bt_address_tospoof):
            _LOGGER.info("Send magic packet to ps4 %s (spoofing ds4: %s) from %s",
                         ps4_address_toconnectto,
                         bt_address_tospoof,
                         adapter)
        else:
            _LOGGER.error("Adapter %s does not yet support addr spoofing",
                          adapter)
    ...
    hass.services.async_register(
        DOMAIN, SERVICE_GET_BT_ADDR, get_bt_addr,
        schema=GET_BT_ADDR_SCHEMA)
